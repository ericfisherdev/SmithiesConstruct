...
import slimeknights.sconstruct.tools.material.ArrowStats;
import slimeknights.sconstruct.tools.material.BowStats;
import slimeknights.sconstruct.tools.material.ExtraStats;
import slimeknights.sconstruct.tools.material.HandleStats;
import slimeknights.sconstruct.tools.material.HeadStats;
import slimeknights.sconstruct.tools.material.Material;
import slimeknights.sconstruct.tools.material.MaterialStats;
...
public final class StatsBuilder {
    ...
    public static ToolStats compute(List<Holder<Material>> mats, ToolModifiers mods, ToolDefinition def) {
        ...
        return slimeknights.sconstruct.tools.trait.TraitRegistry.applyAll(baseline, gatherSlotTraits(materials, def));
    }

    /**
     * Collect every {@link slimeknights.sconstruct.tools.material.MaterialTrait} grant
     ...
     */
    private static java.util.List<slimeknights.sconstruct.tools.material.MaterialTrait> gatherSlotTraits(List<Holder<Material>> materials, ToolDefinition def) {
        java.util.List<slimeknights.sconstruct.tools.material.MaterialTrait> grants = new ArrayList<>();
        for (int i = 0; i < def.getPartCount() && i < materials.size(); i++) {
            ...
            PartType slot = def.getPartSlot(i);
            for (slimeknights.sconstruct.tools.material.MaterialTrait grant : holder.value().traits()) {
                if (grant.slot() == slot) {
                    grants.add(grant);
                }
            }
