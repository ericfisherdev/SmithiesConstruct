package slimeknights.sconstruct.shared.client;
 ...
 * crash on the client-only {@code IClientFluidTypeExtensions} reference. {@link SConstruct}
 * guards the {@link #register(IEventBus)} call with a {@code Dist.CLIENT} check; this class
 * never needs to be loaded on the server.
 *
 * <p>Texture paths are {@code sconstruct:block/fluid/bloodstill} (16×16 still tile) and
 * {@code sconstruct:block/fluid/bloodflow} (16×32 two-frame flowing tile with a {@code .mcmeta}
...
public final class SharedClientFluidTypes {
     ...
     * Called from {@link SConstruct} during mod construction, guarded by a {@code Dist.CLIENT}
