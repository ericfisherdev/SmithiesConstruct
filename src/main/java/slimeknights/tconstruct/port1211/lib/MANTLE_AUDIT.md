...
**Generated from:** `grep -rh '^import slimeknights\.mantle' src/main/java --include='*.java' | grep -v port1211 | sort -u`
...
| Category | Meaning |
|---|---|
| **Vendored** | A direct replacement now lives under `port1211/`. Call sites switch to the replacement class. |
...
|---|---|---|
| `slimeknights.mantle.pulsar.pulse.Pulse` | `port1211.common.pulse.Pulse` | Subsystem interface; lifecycle hooks redesigned for NeoForge event bus instead of Pulsar's annotation scanner |
| `slimeknights.mantle.pulsar.control.PulseManager` | `port1211.common.pulse.PulseLoader` | Boots pulses gated by `Config.PULSES.<flag>` |
| `slimeknights.mantle.network.NetworkWrapper` | `port1211.common.TinkerNetwork` | Wraps NeoForge `PayloadRegistrar`; no SimpleNetworkWrapper analogue in modern NeoForge |
| `slimeknights.mantle.util.LocUtils` | `port1211.lib.util.Util.rl` | One-liner `ResourceLocation` constructor; reused in thousands of legacy call sites |
| (implicit `Lazy<T>` pattern) | `port1211.lib.util.Lazy` | Mantle had no public `Lazy`; vendored under the Mantle umbrella because the pattern shows up across pulses |
| (implicit safe-client pattern) | `port1211.lib.client.SafeClient` | Replaces `slimeknights.mantle.client.SafeClientAccess`; dist-aware `Minecraft`/`ClientLevel`/`Player` accessors |

## Dropped

| Mantle import | Replaced by | Rationale |
|---|---|---|
| `slimeknights.mantle.pulsar.config.ForgeCFG` | NeoForge `ModConfigSpec` (see `port1211.common.config.Config`) | Pulsar's TOML/INI config writer superseded by first-party NeoForge config |
...
|---|---|---|
| `slimeknights.mantle.client.gui.GuiElement` | Phase 6 (smeltery UI) / Phase 9 (rendering) | Vanilla `AbstractWidget` + custom render helpers under `port1211.lib.client.gui` if reuse warrants it |
...
| `slimeknights.mantle.inventory.SlotOut` | Phase 5 / Phase 6 | Output-only `Slot` subclass (~5 lines); vendor in `port1211.lib.inventory` only if it appears 3+ times |
...
- **Vendoring budget:** the foundation plan caps `port1211.lib.*` at roughly 20 files. Before adding a Vendored row, confirm the helper is referenced in ≥3 call sites; otherwise inline the logic and skip the wrapper.
