...
- **Spotless** formats `src/main/java/slimeknights/sconstruct/**` and `src/test/java/**` against the modernized `eclipse_formatter.xml` (K&R braces, modern method-paren spacing) plus a stable import order. The pre-commit hook runs `spotlessCheck`; if it fails, run `./gradlew spotlessApply` to auto-fix, re-stage, and re-commit.
